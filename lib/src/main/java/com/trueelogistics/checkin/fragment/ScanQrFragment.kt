...
class ScanQrFragment : androidx.fragment.app.Fragment() {
    ...
    fun bindingData() {
        setDisableBackPage()
        type_check_in.text = when (arguments?.getString(KEY_TYPE_SCAN_QR).toString()) {
    ...
    private fun setDisableBackPage() {
        if (arguments?.getBoolean(KEY_DISABLE_BACK, false) == true) {
            disableBack()
        }
    }
    ...
    fun disableBack() {
        this.back_page.visibility = GONE
    }

    fun enableBack() {
        this.back_page.visibility = VISIBLE
    }
